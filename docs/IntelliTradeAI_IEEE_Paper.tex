...
We design our custom backtesting engine to evaluate strategy performance through walk-forward optimization. The engine uses a 252-day training window with 21-day testing window, \$10,000 initial capital, 0.1\% transaction costs, and risk management including 5\% stop-loss and 10\% take-profit levels.
...
To validate that our results are not due to random chance, we perform paired t-tests comparing our tri-signal fusion against baseline models. Table~\ref{tab:significance} presents the statistical significance results. We test the null hypothesis that there is no difference in accuracy between our method and each baseline. A p-value below 0.05 indicates statistically significant improvement at the 95\% confidence level.
...
\bibitem{b4} Cheng, L., et al., ``Financial time series forecasting with multi-modality graph neural network,'' \textit{Pattern Recognition}, vol. 121, 108215, 2022.
\bibitem{b5} Chen, T., and Guestrin, C., ``XGBoost: A scalable tree boosting system,'' \textit{Proc. 22nd ACM SIGKDD}, pp. 785--794, 2016.
\bibitem{b6} Yanxi, W., ``Deep learning for financial time series prediction,'' \textit{Expert Systems with Applications}, vol. 184, 115436, 2021.
...
\bibitem{b21} Sezer, O. B., Gudelek, M. U., and Ozbayoglu, A. M., ``Financial time series forecasting with deep learning,'' \textit{Applied Soft Computing}, vol. 90, 106181, 2020.
...
\bibitem{b23} Kraaijeveld, O., and De Smedt, J., ``The predictive power of public Twitter sentiment for forecasting cryptocurrency prices,'' \textit{Journal of International Financial Markets}, vol. 65, 101188, 2020.
