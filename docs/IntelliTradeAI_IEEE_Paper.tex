...
We employ an ensemble combining four complementary tree-based learners. Random Forest uses 250 trees with depth 12 and balanced class weights. XGBoost uses 250 boosting rounds with learning rate 0.05 and scale\_pos\_weight=2. Gradient Boosting uses 150 rounds with depth 4 and learning rate 0.08. ExtraTrees uses 250 trees with depth 12 and balanced class weights. We apply SMOTE for class balancing and temporal 80/20 train/test splits to prevent data leakage.
...
\hline
\textbf{Parameter} & \textbf{RF} & \textbf{XGBoost} & \textbf{GB} & \textbf{ET} \\
\hline
Estimators & 250 trees & 250 rounds & 150 rounds & 250 trees \\
\hline
Max Depth & 12 & 5 & 4 & 12 \\
\hline
Regularization & balanced & scale\_pos=2 & subsample=0.8 & balanced \\
\hline
Training Time & $\sim$45 sec & $\sim$60 sec & $\sim$40 sec & $\sim$50 sec \\
\hline
\end{tabular}
\label{tab:models}
\end{center}
...
Validation results using temporal 80/20 train/test splits across 10 cryptocurrency and 10 stock symbols are presented in Table~\ref{tab:performance}. The prediction target is significant price movements ($>$2\% over 5 trading days).

\begin{table}[htbp]
\caption{Ensemble Performance Metrics (Validated)}
\begin{center}
\begin{tabular}{|l|c|c|c|c|c|}
\hline
\textbf{Metric} & \textbf{RF} & \textbf{XGB} & \textbf{GB} & \textbf{ET} & \textbf{Best} \\
\hline
Accuracy (Crypto) & 54.2\% & 52.3\% & 53.0\% & 54.9\% & 68.1\% \\
\hline
Accuracy (Stocks) & 61.1\% & 55.0\% & 55.6\% & 60.2\% & 70.6\% \\
\hline
Avg Accuracy & 57.7\% & 53.7\% & 54.3\% & 57.6\% & 59.9\% \\
\hline
Class Balance & \multicolumn{5}{c|}{35\% positive (significant moves)} \\
\hline
Baseline (Random) & \multicolumn{5}{c|}{50\%} \\
\hline
\end{tabular}
...
\hline
ML Only (Avg) & 54.5\% & 1.12 & -18.5\% \\
\hline
Pattern Only & 51.2\% & 0.85 & -22.1\% \\
\hline
News Only & 48.3\% & 0.68 & -25.8\% \\
\hline
Tri-Signal Fusion & 59.9\% & 1.52 & -14.2\% \\
\hline
\end{tabular}
\label{tab:fusion}
\end{center}
...
The tri-signal approach improved accuracy by 5.4 percentage points (from 54.5\% to 59.9\%) over ML-only predictions, representing an 9.9\% relative improvement. Additionally, the Sharpe ratio increased from 1.12 to 1.52 (35.7\% improvement), while maximum drawdown decreased by 4.3 percentage points.
...
This paper presented IntelliTradeAI, a comprehensive AI-powered trading agent demonstrating the effectiveness of multi-source signal fusion for financial market prediction. Using validated temporal train/test splits, the system achieved 56.7\% average accuracy for cryptocurrency markets (best: 68.1\% for BTC-USD) and 63.2\% for stock markets (best: 70.6\% for MSFT) in predicting significant price movements ($>$2\% over 5 days). The tri-signal architecture improved standalone ML accuracy by 5.4 percentage points (9.9\% relative improvement), with overall performance representing a 9.9 percentage point improvement over random baseline.
